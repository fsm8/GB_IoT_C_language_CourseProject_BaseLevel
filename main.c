...
int main(int argc, char* argv[])
{
	...
	if (user_filename[0] == '\0')
	{
		...
		printf("Для указания файла используйте ключ -f, например .\prog.exe -f file.csv\n");
		printf("Для выхода из программы нажмите Enter\n");
		while (getchar() != '\n');
		return 1;
	}
	...
	else // Выводим статистику по месяцам
	{
...
void print_help(void)
{
	...
	printf("Список подерживаемых программой ключей:\n");
	printf("1. -h - список ключей, которые обрабатывают данное приложение и их назначение.\n");
